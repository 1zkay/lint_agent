...
- The same root pollutes multiple child modules, multiple outputs, or multiple internal control paths.
- The polluted targets are control signals, state-update enables, mux selects, protocol signals, or module outputs, not only harmless data constants.
- The polluted signals should normally depend on runtime state, debug state, exception state, branch state, valid/ready flow, or protocol handshakes.
...
- The polluted signals are exactly the signals that implement the disabled feature or fixed configuration.
...
- If the root were restored to non-constant, would the polluted outputs or control paths become behaviorally meaningful again?
...
- One or two representative constant outputs, inputs, or non-port signals from the polluted set.
